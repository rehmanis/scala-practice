...
trait DecoderInstances {
  ...
  implicit val boolDecoder: Decoder[Boolean] =
    Decoder.fromPartialFunction { case Json.Bool(value) => value.booleanValue }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(items) => {
        val itemsList = items.map(decoder.decode)
        if (itemsList.forall(_.isDefined)) Some(itemsList.map(_.get))
        else None
      }
      case _ => None
    }
